...
wget --inet4-only --https-only https://raw.githubusercontent.com/badmojr/1Hosts/refs/heads/master/Lite/hosts.txt -O /etc/hosts-badmojr
wget --inet4-only --https-only https://github.com/hagezi/dns-blocklists/raw/refs/heads/main/hosts/ultimate.txt -O /etc/hosts-hagezi-ultimate
wget --inet4-only --https-only https://big.oisd.nl/domainswild2 -O /etc/hosts-oisd
wget --inet4-only --https-only https://github.com/badmojr/1Hosts/raw/refs/heads/master/Xtra/hosts.txt -O /etc/hosts-1hosts-xtra
...
cat /etc/hosts-badmojr >> /etc/hosts-ad-blocker
